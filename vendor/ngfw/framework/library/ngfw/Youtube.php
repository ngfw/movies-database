<?php
// Based on https://github.com/madcoda/php-youtube-api 
// original author Madcoda
// The MIT License (MIT)
// 
namespace ngfw;
...
class Youtube
{
    ...
    public function __construct($params)
    {
        if (is_array($params) && array_key_exists('key', $params)) {
            ...
            $this->youtube_key = $params;
        }
    }

    public function getEntry($vId){
        return $this->getVideoInfo($vId);
    }
    /**
     * @param $vId
     * @return \StdClass
     ...
     * @return array
     */
    public function search($q, $maxResults = null)
    {
        if(empty($maxResults)){
     ...
     */
    public function searchVideos($q, $maxResults = null, $order = null)
    {
        if(empty($maxResults)){
            $maxResults = $this->maxResults;
        }
     ...
     * @return object
     */
    public function searchChannelVideos($q, $channelId, $maxResults = null, $order = null)
    {
        if(empty($maxResults)){
            $maxResults = $this->maxResults;
        }
    ...
    public function getRelated($videoID){
        $APIrelatedVideosIDs = $this->api_get($this->getApi("search.list"), 
            array(
                'key' => $this->youtube_key, 
        ...
        $relatedVideosIDs = $this->decodeList($APIrelatedVideosIDs);
        
        
        if (isset($relatedVideosIDs) and is_array($relatedVideosIDs)):
            return $relatedVideosIDs;
        endif;
        return false;
    }
    ...
    public function setStart($pageToken){
        $this->pageToken = $pageToken;
    }

    public function getChannel(){
        return $this->getChannelByName($this->author, array(
            "maxResults" => $this->maxResults,
    ...
    public function api_get($url, $params)
    {
        ...
        $tuCurl = curl_init();
        //\ngfw\App::debug($url . (strpos($url, '?') === false ? '?' : '') . http_build_query($params));
